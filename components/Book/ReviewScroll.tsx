...
interface Post {
    userId: string;
    review: string | null; // Permette che review possa essere null
    username: string;
}
...
const ReviewScroll = ({ post }: Props) => {
    ...
    useEffect(() => {
        ...
        if (scrollContainer) {
            ...
            const scrollInterval = setInterval(autoScroll, 80); 
    ...
    const truncatereview = (review: string | null, length: number = 60) => {
        ...
        <div className="p-auto m-auto flex flex-col bg-white mt-12">
            ...
            <div className="hide-scroll-bar flex overflow-x-scroll pb-10 " id="scroll-container">
                <div className="ml-10 flex flex-nowrap md:ml-20 lg:ml-40">
                    {post
                        .filter(item => item.review && item.review.trim() !== '') // Filtra le recensioni null o vuote
                        .map((item, index) => (
                            <div key={index} className="inline-block px-3 ">
                                <div className="h-48 w-96 flex justify-start items-center max-w-xs overflow-hidden rounded-lg bg-zinc-50 shadow-lg transition-shadow duration-300 ease-in-out hover:shadow-xl">
                                    <div className="flex flex-col space-y-2 p-5">
                                        <p>"{truncatereview(item.review)}"</p>
